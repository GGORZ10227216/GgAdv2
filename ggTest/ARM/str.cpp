...
TEST_F(ggTest, arm_str_post_reg_offset_test) {
  ...
  auto TestMain = [&]() {
	...
	case LSL:offset <<= shiftAmount.value;
	  break;
	case LSR:offset >>= shiftAmount.value;
	  break;
	case ASR:offset = static_cast<int32_t>(offset) >> shiftAmount.value;
	  break;
	case ROR:offset = rotr(offset, shiftAmount.value);
	  break;
	}
