...
//            fmt::print("[{}] LDR base: {:#x} offset: {:#x} total_t: {}\r", std::this_thread::get_id(), RbValue, RoValue, t);
  ...
  const auto areaList = gg_core::make_array(
	  std::make_tuple(BIOS_start, E_BIOS_SIZE, onboardStart),
	  std::make_tuple(onboardStart, E_EWRAM_SIZE, onchipStart),
	  std::make_tuple(onchipStart, E_IWRAM_SIZE, ioStart),
	  std::make_tuple(paletteStart, E_PALETTE_SIZE, VRAM_Start),
	  std::make_tuple(VRAM_Start, E_VRAM_SIZE, OAM_Start),
	  std::make_tuple(OAM_Start, E_OAM_SIZE, WAITSTATE_0_Start),
	  std::make_tuple(WAITSTATE_0_Start, E_ROM_BLOCK_SIZE, WAITSTATE_1_Start),
	  std::make_tuple(WAITSTATE_1_Start, E_ROM_BLOCK_SIZE, WAITSTATE_2_Start)
//            std::make_tuple(WAITSTATE_2_Start, E_ROM_BLOCK_SIZE, SRAM_Start) // bypass eeprom test
  );
...
//            TestMain(0x2000000, 1, 0x03000000) ;
  } // for
}

TEST_F(ggTest, Thumb_load_byte_with_reg_offset) {
  auto TestMain = [&](uint32_t RbValue, uint32_t RoValue, uint32_t areaBound) -> uint64_t {
	uint64_t t = 0;
	  ...
	  for (int RbNum = 0; RbNum < 8; ++RbNum) {
		for (int RoNum = 0; RoNum < 8; ++RoNum) {
		  for (int RdNum = 0; RdNum < 8; ++RdNum) {
			...
			uint16_t instruction = (0b0101 << 12) | (0b11 << 10) | (RoNum << 6) | (RbNum << 3) | RdNum;
			...
			ASSERT_TRUE((local_cpu.lastCallee == LoadStoreRegOffset<true, true>));
			ASSERT_TRUE(errFlag == 0)
						<< "#" << t << " of test\n"
						<< std::hex << "Errflag: " << errFlag << '\n'
						<< input
...
TEST_F(ggTest, Thumb_store_with_reg_offset) {
	  ...
	  for (int RbNum = 0; RbNum < 8; ++RbNum) {
		for (int RoNum = 0; RoNum < 8; ++RoNum) {
		  for (int RdNum = 0; RdNum < 8; ++RdNum) {
			...
			uint16_t instruction = (0b0101 << 12) | (RoNum << 6) | (RbNum << 3) | RdNum;
			...
			uint32_t ggReadBack = gbaInstance.mmu.Read<uint32_t>(targetDst, S_Cycle);

			uint32_t errFlag = CheckStatus(local_cpu, arm);
			std::string input = fmt::format("Base RegValue: {:#x}, Offset RegValue: {:#x}\n",
											RbValue, RoValue);

			ASSERT_TRUE((local_cpu.lastCallee == LoadStoreRegOffset<false, false>));
...
TEST_F(ggTest, Thumb_store_byte_with_reg_offset) {
  auto TestMain = [&](uint32_t RbValue, uint32_t RoValue, uint32_t RdValue, uint32_t areaBound) -> uint64_t {
	uint64_t t = 0;
	  ...
	  for (int RbNum = 0; RbNum < 8; ++RbNum) {
		for (int RoNum = 0; RoNum < 8; ++RoNum) {
		  for (int RdNum = 0; RdNum < 8; ++RdNum) {
			...
			uint16_t instruction = (0b0101 << 12) | (0b1 << 10) | (RoNum << 6) | (RbNum << 3) | RdNum;
			...
			ASSERT_TRUE((local_cpu.lastCallee == LoadStoreRegOffset<false, true>));
			ASSERT_TRUE(refReadBack == ggReadBack)
						<< fmt::format("#{} mine: {:#x} ref: {:#x}, offset: {:#x}",
									   t,
									   ggReadBack,
									   refReadBack,
									   RoValue);
