...
TEST_F(ggTest, Thumb_load_halfword_imm_offset
) {
	...
	for (int RbNum = 0; RbNum < 8; ++RbNum) {
	  for (int offset = 0; offset < 0x20; ++offset) {
		for (int RdNum = 0; RdNum < 8; ++RdNum) {
		  uint16_t instruction = (0b1000 << 12) | (0b1 << 11) | (offset << 6) | (RbNum << 3) | RdNum;
		  ...
		  uint32_t targetDst = local_cpu._regs[RbNum] + (offset << 1);
		  ...
		  ASSERT_TRUE((local_cpu.lastCallee == LoadStoreImmOffsetHalf < true > ));
...
TEST_F(ggTest, Thumb_store_halfword_imm_offset
) {
auto TestMain = [&](uint32_t RbValue, uint32_t RdValue, uint32_t areaBound) -> uint64_t {
  ...
  auto task = [&]() {
	for (int RbNum = 0; RbNum < 8; ++RbNum) {
	  for (int offset = 0; offset < 0x20; ++offset) {
		for (int RdNum = 0; RdNum < 8; ++RdNum) {
		  uint16_t instruction = (0b1000 << 12) | (offset << 6) | (RbNum << 3) | RdNum;
		  ...
		  ASSERT_TRUE((local_cpu.lastCallee == LoadStoreImmOffsetHalf < false > ));
