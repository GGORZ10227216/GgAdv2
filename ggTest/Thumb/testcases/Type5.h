...
void Type5CalleeCheek(CPU &local_cpu, const unsigned Op, const bool H1, const bool H2) {
  ...
  switch (hashcode) {
  case 0b0001:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<ADD, false, true>));
	break;
  case 0b0010:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<ADD, true, false>));
	break;
  case 0b0011:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<ADD, true, true>));
	break;

  case 0b0101:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<CMP, false, true>));
	break;
  case 0b0110:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<CMP, true, false>));
	break;
  case 0b0111:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<CMP, true, true>));
	break;

  case 0b1001:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<MOV, false, true>));
	break;
  case 0b1010:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<MOV, true, false>));
	break;
  case 0b1011:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<MOV, true, true>));
	break;

  case 0b1100:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<0, false, false>));
	break;
  case 0b1101:ASSERT_TRUE((local_cpu.lastCallee == HiRegOperation_BX<0, false, true>));
	break;
...
TEST_F(ggTest, Thumb_HiReg) {
  ...
  TestMain(0, true, false, 0, 0x04040404);
}

TEST_F(ggTest, Thumb_BX_Lo) {
  auto TestMain = [&](const unsigned Op, bool HiRd, bool HiRs, const uint32_t RsValue,
					  const uint32_t RdValue) -> uint64_t {
	uint64_t t = 0;
